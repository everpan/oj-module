...
#   ojm build → ojm preview 冒烟 → ojm dev 冒烟
# 每态：后台拉起 ojm，轮询 /api/health 直到就绪，跑 scripts/smoke_all.py，
# 再 SIGINT + 兜底 pkill 清理（oj 与 ojm 子进程不残留占端口）。
# 任一态失败即非零退出。
set -uo pipefail
...
run_state() {
  ...
  # 清理：先 SIGINT 让 ojm 优雅回收 oj，再兜底强杀残口
