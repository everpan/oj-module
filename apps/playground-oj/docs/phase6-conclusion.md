...
| 阶段 | 计划目标 | 状态 | 交付 / 提交 |
|---|---|---|---|
| P0 | 骨架 + oj 登录冒烟 + `ojm dev` 反代 | ✅ | `4a38611` |
...
- **P0 骨架**：`ojm init` 生成工程，端口改 9779（避开 playground 的 9778），依赖回 `workspace:*`/`catalog:`，oj 登录链与 `ojm dev` 反代联调全绿。
- **P1 注入与契约**：运行时 D9 三套注册表（systemApi / notificationsApi / uploadApi）+ `ojm api` 契约生成与豁免 + 11 模块自包含，playground 可独立跑自己的 oj 后端。
...
- **P5 release 回归**：`ojm build` 抓出 S003 跨模块 `deps` 缺失、`pnpm typecheck` 抓出 `home/line` evolving-array 编译错误；`scripts/smoke.sh` 一键跑通 build→preview→dev 两态冒烟（均 10/10）。
...
- **R4 契约对账 ≠ 编译**：`ojm api --check` 只校验生成物同步 / route 双向 / routes.js 漂移，**不编译 handler**；`pnpm typecheck` 才是 handler 的 tsc 关卡，两者必须各跑各的（P5）。
...
- **C10 后台进程生命周期在调用方手里**：长驻服务（`ojm dev`/`preview` 起的 oj + 静态）必须"起—测—停"放进**同一条命令**，跨 Bash 调用会被回收，出现"上条起了服务、下条连不上"的诡异失败（P5）。
...
playground-oj 从 `ojm init` 骨架出发，分六阶段走完了"模块自包含 → 认证与数据层 → system CRUD 接管 → 业务模块真实化 → release 回归 → 集成冒烟"的完整链路：
...
- **质量闭环**：`ojm api --check` 0/0、两态（dev + preview）集成冒烟 10/10、`pnpm typecheck` 绿、`ojm build`/`ojm preview`/`ojm api --docs` 全绿；`scripts/smoke.sh` 可一键复跑回归。
