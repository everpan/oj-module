...
> 阶段目标（计划 §5）：`ojm build` / `ojm preview` / `ojm api --docs` 全绿；`scripts/smoke.sh` 对 dev + preview 两态做端到端冒烟；root-app 回归（typecheck）无回退。
>
...
| 步骤 | 动作 | 关键产物 | 验证 |
|---|---|---|---|
| P5-1a | `ojm build`：oj build（后端 6 模块）→ 全站合并 → 10 模块构建 → modules.json | `api/dist/manifests.yaml` + `modules/dist/*` | 构建成功 |
| P5-1b | **修复 S003 闸门**：`web/manifest.yaml` 声明 `deps:{system:^0.1.0}`（`get-async-routes` 跨模块读 menus/role_menu） | `api/src/web/manifest.yaml` | `ojm build` 不再报 ownership_guard 错误 |
| P5-2 | `ojm api --docs`：生成文档站 + 每模块 `openapi.yaml` | `api/docs/index.html` + `api/src/<m>/openapi.yaml` | 文档站生成 |
| P5-3 | preview 态：`ojm preview` 起 oj(release)+静态(4173)，跑 `scripts/smoke_all.py` | 终端 | 10/10 PASS |
| P5-4 | dev 态：`ojm dev` 起 oj(src)+静态(5174)，跑 `scripts/smoke_all.py` | 终端 | 10/10 PASS |
...
`web` 模块的 `get-async-routes` 跨模块读 `system` 的 `menus`/`role_menu`（P2-2 真实化时已知，注释里写了"生产可声明 deps"）。`ojm build` 初跑即报：
...
修法：在 `api/src/web/manifest.yaml` 加 `deps: { system: "^0.1.0" }`。重跑 `ojm build` 通过——6 个后端模块编译进 `api/dist`，全站合并进 `modules/dist`，10 个前端模块构建生成 `modules.json`。
...
[ojm] 已合并宿主站点 → modules/dist
[ojm] 构建 about/access/exception/home/outside/personal-center/route-nest/system/demo/login/notification @...
[ojm] 清单已生成 → modules/dist/modules.json
```
...
2. `run_state preview 4173`：后台拉起 → 轮询 `/api/health` 至 200 → 冒烟 → SIGINT 回收 ojm（ojm 优雅停 oj）→ 兜底 `pkill bin/oj` + 强杀残口；
3. `run_state dev 5174`：同上，前态清理后再起，避免 9779 端口冲突（dev/preview 的 oj 都用 config 里的同一端口）。

> 进程清理要点：dev/preview 各自 spawn 一个 oj 子进程（config 钉死 9779）。两态顺序跑、前态必须彻底清理（ojm SIGINT 仅回收自己的 oj，再用 `pkill` 兜底），否则后态 oj 因端口占用 `ready` 拒绝、ojm fail-fast。
...
6. **`ojm preview` 自带 migrate**：preview 起服务前先 `oj migrate` 应用待执行迁移，dev 则靠 `oj server` 启动期 reconcile；两态都不需要手动 migrate，但 release 产物要求先 `ojm build`（产出 `api/dist/manifests.yaml`）。
7. **`ojm api --check` 不编译 handler，只对账契约**：它查生成物同步 / route 双向 / routes.js 漂移，但**不跑 tsc**。P4 的 `home/line` handler 里 `const out: number[] = []; out.push(map.get(d) ?? 0)` 触发 TS「evolving array」推断——`?? 0` 的 `0` 字面量把 `out` 收窄成 `0[]`，后续 `out.push(number)` 报 `number 不可赋值给 0`。`pnpm typecheck` 才是 handler 编译关卡，必须单独跑。修法：用 `Array.from(..., cb)` 产出明确 `number[]`，绕开 evolving 推断。
8. **`ojm build` 编译的是工作树源码，能否进提交另算**：P4 提交时 `personal-center/upload/api.ts` 误夹带了 `ojm api` 生成的 stub（`json.ok("示例")`），真实 multipart→base64 实现只在工作树、未入库。P5 经 `git commit --amend` 把真实实现补进 P4 提交，使提交与「头像上传真实化」的承诺一致（教学历史要可复现）。

## 阶段小结

- `ojm build` 初跑即暴露 S003 跨模块依赖缺失：`web` 读 `system` 表未声明 `deps`，补 `deps:{system:^0.1.0}` 后通过。这是 P5 回归抓到的真实 release 风险。
- `ojm api --docs` 生成文档站与 openapi；`ojm build` 产出 api/dist + 合并全站 + modules.json。
