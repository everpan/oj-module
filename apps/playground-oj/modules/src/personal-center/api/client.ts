...
export type UploadData = z.infer<(typeof schemas)["upload"]["data"]>;

export async function upload(): Promise<UploadData> {
	const client = ensureReq();
	try {
		const env = await client.post(`personal-center/upload`).json<OjEnvelope<UploadData>>();
		...
		const data = env.data as UploadData;
		if (import.meta.env.DEV) {
			const { schemas } = await import("./client.schemas");
			const r = schemas.upload.data.safeParse(data);
			if (!r.success)
				throw new ContractApiError(-1, `[契约违例] upload 响应与契约不符：${r.error.issues.map(i => `${i.path.join(".") || "(root)"}: ${i.message}`).join("; ")}`);
		}
		return data;
	}
	catch (e) {
