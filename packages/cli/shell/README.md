...
| | |
|---|---|
| 消费者 | `@oj-module/cli`（`ram dev` / `ram build`） |
...
```bash
pnpm --filter @oj-module/shell build   # 全量构建 dist/
pnpm --filter @oj-module/shell dev     # 同上，watch 模式
```

`scripts/build.mts` 会先构建 `@oj-module/runtime` 并把 `runtime.js` 拷进 `assets/`，随后逐个构建共享依赖入口，最后构建宿主并注入 importmap。
